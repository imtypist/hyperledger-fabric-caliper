...
function createOrgs() {
  ...
  ./organizations/ccp-generate.sh $NUM_OF_PEER
}
...
function networkUp() {
  ...
  COMPOSE_FILES="-f compose/${NUM_OF_PEER}-${COMPOSE_FILE_BASE} -f compose/${CONTAINER_CLI}/${NUM_OF_PEER}-${CONTAINER_CLI}-${COMPOSE_FILE_BASE}"
...
function createChannel() {
  ...
  if [[ $len -ge $((NUM_OF_PEER+2)) ]] && [[ ! -d "organizations/peerOrganizations" ]]; then
    echo "Bringing network down to sync certs with containers"
    networkDown
  fi

  [[ $len -lt $((NUM_OF_PEER+2)) ]] || [[ ! -d "organizations/peerOrganizations" ]] && bringUpNetwork="true" || echo "Network Running Already"
  ...
  scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $NUM_OF_PEER
}


## Call the script to deploy a chaincode to the channel
function deployCC() {
  #echo "Num of peer: "$NUM_OF_PEER
  scripts/deployCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE $NUM_OF_PEER
...
function networkDown() {
  ...
  COMPOSE_BASE_FILES="-f compose/${NUM_OF_PEER}-${COMPOSE_FILE_BASE} -f compose/${CONTAINER_CLI}/${NUM_OF_PEER}-${CONTAINER_CLI}-${COMPOSE_FILE_BASE}"
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    for((i=1; i<=$NUM_OF_PEER; i++))
    do
      ${CONTAINER_CLI} volume rm docker_peer0.org${i}.example.com
    done
